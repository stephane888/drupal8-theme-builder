...
/* controls */    	
	...
	loadUrl : function(url, callback) {	console.log('loadUrl '); //console.log(url);
		jQuery("#select-box").hide();
		
		self.initCallback = callback; //console.log(self.iframe.src);
    ...
    _initHighlight: function() {
		...
		this.frameBody.on("dblclick", function(event) {
			...
			if (Vvveb.Builder.isPreview == false)
			{
				...
				/// chech image
				chekcImage(event, this);
			}
		});
		function chekcImage(event, iframe){
		  //console.log(event);
		  //console.log(iframe);		  
		  if(event.target.localName == 'img'){
		    //console.log($('img', iframe));
		    $('img', iframe).removeAttr('data-img-drupalwbu');
		    $(event.target).attr('data-img-drupalwbu', 'drupal-image-replace');
		    $('#drupal-images').modal('show');
...
/* drag and drop */
	...
	{ console.log(window.FrameDocument);
		//update only body to avoid breaking iframe css/js relative paths
		start = html.indexOf("<body"); //console.log(start)
    end = html.indexOf("</body");		//console.log(end);
	...
	setHtml25: function(html){
	  //console.log("setHtml25 : "+html);
	  //console.log($('#iframe-wrapper > iframe').contents().find('body'));
	  $('#iframe-wrapper > iframe').contents().find('body').html(html);
	}
};
...
Vvveb.Gui = {
	...
	undo : function () {
	  console.log('data-vvveb-action undo');
	...
	redo : function () {
	  console.log('data-vvveb-action redo');
		if (Vvveb.WysiwygEditor.isActive) 
		{
	...
	save : function () {
	  console.log('data-vvveb-action save');
		$('#textarea-modal textarea').val(Vvveb.Builder.getHtml());
		$('#textarea-modal').modal();
	},
	
	download : function () {
	  console.log('data-vvveb-action download');
	...
	viewport : function () {
	  console.log('data-vvveb-action viewport');
		$("#canvas").attr("class", this.dataset.view);
	},
	
	toggleEditor : function () {
	   //console.log('data-vvveb-action toggleEditor');
	...
	toggleEditorJsExecute : function () {
	  console.log('data-vvveb-action toggleEditorJsExecute');
		Vvveb.Builder.runJsOnSetHtml = this.checked;
	},
	
	preview : function () {
	  console.log('data-vvveb-action preview');
	...
	fullscreen : function () {
	  console.log('data-vvveb-action fullscreen');
		launchFullScreen(document); // the whole page
	},
	
	componentSearch : function () {
	  console.log('data-vvveb-action componentSearch');
